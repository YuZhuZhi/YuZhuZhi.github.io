...
$
P(j omega)= omega_s  sum_(k=- infinity)^(+ infinity) delta( omega-k omega_s)
$
...
也就是说，在采样之后，原信号的频谱 $X(j omega)$ 会复制无穷多份，而这每一份会(在频域上)被等间隔地平移 $ omega_s$ ，之后再将这些被平移的频谱都加在一起(并附上系数 $ frac(1, T)$ )，即形成了采样信号 $x_p(t)$ 的频谱。
...
$
X(j omega)=0, quad abs(omega) >=  omega_M
$
...
两者没有重叠的条件是 $ omega_s- omega_M >=  omega_M$ ，即 $ omega_s >=2 omega_M$ 。此时每个相邻的 $X(j omega)$ 彼此分离，使用截止频率为 $ omega_M$ 的低通滤波器就能得到原信号频谱 $X(j omega)$ ——也就是说，能够完全地恢复时域上的原信号！
