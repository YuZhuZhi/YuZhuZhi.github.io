...
== 3.变中心频率的频率选择性滤波器 <变中心频率的频率选择性滤波器>
...
假设低通滤波器的截止频率是 $ omega_c$ 。那么，将原信号乘上 $te^(tj omega_0t)$ ，就使原信号的频谱左移了 $ omega_0$ 。此时通过低通滤波器，就会留下原信号 $[ omega_0- omega_c, omega_0+ omega_c]$ 区间的频率(但此时处于 $[- omega_c, omega_c]$ 的位置)。因此之后再乘上 $te^(-tj omega_0t)$ ，就将频谱移回原位。
...
为了求出 $x_p(t)$ 的傅里叶变换，不妨利用信号相乘性质。为此，查表得到 $p(t)$ 的傅里叶变换为#footnote[记 $ omega_s= frac(2 pi, T)$ ，称为*采样频率*。实际求取时应使用周期函数的傅里叶变换]：

$
P(tj omega)= omega_s  sum_(k=- infinity)^(+ infinity) delta( omega-k omega_s)
$

所以：

$
    X_p(tj omega)&= frac(1, 2 pi)X(tj omega)*P(tj omega)\
    &= frac(1, 2 pi)  integral_(- infinity)^(+ infinity)X(j phi)P(j( phi- omega))d phi\
    &= frac(omega_s, 2 pi) sum_(k=- infinity)^(+ infinity) integral_(- infinity)^(+ infinity)X(j( omega+k omega_s)) delta( phi- omega-k omega_s)d phi\
    &= frac(1, T) sum_(k=- infinity)^(+ infinity)X(j( omega+k omega_s))
$

也就是说，在采样之后，原信号的频谱 $X(tj omega)$ 会复制无穷多份，而这每一份会(在频域上)被等间隔地平移 $ omega_s$ ，之后再将这些被平移的频谱都加在一起(并附上系数 $ frac(1, T)$ )，即形成了采样信号 $x_p (t)$ 的频谱。

== 2.采样定理

大部分信号的频谱会由负无穷延伸到正无穷，这时采样信号 $x_p (t)$ 的频谱必然是原信号频谱 $X(tj omega)$ 的层层叠叠，称为*混叠*。此时，我们基本不可能从采样信号的频谱中提取出原信号的频谱。
...
$
X(tj omega)=0, quad abs(omega) >=  omega_M
$

如同之前所说，当原信号频谱 $X(tj omega)$ 平移之后叠加，若之间有重叠部分，那么我们就不能从中恢复 $X(tj omega)$ 。但带限信号却提供了恢复原信号频谱的可能性，只要相邻两个平移的 $X(tj omega)$ 之间*没有重叠*即可。比如，取如下两个：

$
X(tj omega) quad, quad X(tj( omega- omega_s))
$

两者没有重叠的条件是 $ omega_s- omega_M >=  omega_M$ ，即 $ omega_s >=2 omega_M$ 。此时每个相邻的 $X(tj omega)$ 彼此分离，使用截止频率为 $ omega_M$ 的低通滤波器就能得到原信号频谱 $X(tj omega)$ ——也就是说，能够完全地恢复时域上的原信号！

这就是所谓*采样定理*，即当*采样频率*满足 $ omega_s >=2 omega_M$ 时，对带限信号冲激采样形成离散信号后，依然能完整地恢复原信号。
...
$
c(t)=cos   omega_c t= frac(1, 2)te^(tj omega_c t)+ frac(1, 2)te^(-tj omega_c t)
$
...
@变中心频率的频率选择性滤波器 中例子的后半部分就是以复指数函数作为载波信号的信号解调了。乘上 $te^(-tj omega_c t)$ 即可使其频率降低 $ omega_c$ ，恢复为原信号。
...
#tufted.remark[][
相位差会使解调得到的信号在幅度上产生衰减，尤其是当相位差为 $ frac(pi, 2)$ 时得到恒零信号。因此这种解调方法要得到最好效果，就需使解调用信号与载波信号完全同步，而这在现实中并不好实现。
]
