...
以往我们总是见到“一个点处在平面 $R times R$ 上”之类的表述，粗略地理解为一个由两个垂直的实数轴张成的平面。*笛卡尔积*实际是生成一个以*有序对*作为元素的集合，放到平面几何中来说，上述的粗略理解倒也正确。

#tufted.definition[有序对][
有序对由两个元素 $a,b$ 构成，记成 $ chevron.l a,b chevron.r$。与集合的无序性不同，尖括号内元素的次序会影响结果。准确地说：

$
a != b <-> chevron.l a,b chevron.r != chevron.l b,a chevron.r
$
]

#tufted.remark[有序对的另一种记法][
有序对也可以用圆括号书写，比如 $(a,b)$，这就是通常见到的“坐标”形式。
]

而两个集合 $A,B$ 间的笛卡尔积记成 $A times B$ ，定义为：

$
A times B= { chevron.l a,b chevron.r | a in A and b in B }
$

显然在大多数情况下 $A times B != B times A$ 。笛卡尔积既不满足*交换律*也不满足*结合律*。但是，笛卡尔积*与集合并有分配律*：
...
一个*关系*我们通常用大写字母 $R$ 来表示，它亦是一种*集合*，取的是两个集合的笛卡尔积的*子集*，即 $R subset.eq A times B$ 。如果 $A=B$ ，那么我们称 $R$ 是集合 $A$ 上的关系。

对于 $A times B$ 中的元素 $ chevron.l a,b chevron.r$ ，如果它是 $R$ 中的元素，那么就称 $a$ 和 $b$ *具有关系* $R$ ，可以记为 $a  R  b$ 。可以结合比较常见的关系——比如实数域上的相等 $=$ ——来理解。而从“相等”这个概念出发，我们引申出*恒等关系*，这种关系只存在于一个集合 $A$ 及其自身的笛卡尔积 $A times A$ 中，记作 $ Delta_A$ ：

$
 Delta_A= { chevron.l a,a chevron.r | a in A }
$

关系除了可以用集合的表示方法来表示之外，在元素较少的情况下也可以用*有向图*来表示。例如，有序对 $ chevron.l 3,4 chevron.r$ 在图中表示为顶点 $3$ 指向顶点 $4$ 的一条*有向边*。当然我们也能用更加“数学化”的*关系矩阵*来描述。假设集合 $A= {a_1,a_2,...,a_n },B= {b_1,b_2,...,b_k }$ ，而它们有一个关系 $R subset.eq A times B$ ，由此唯一确定一个 $n times k$ 的布尔矩阵 $ bold(M)_R$ 。记这个矩阵第 $i$ 行 $j$ 列的元素为 $m_(i j)$ ，那么若 $a_i$ 与 $b_j$ 具有关系 $R$ ，则 $m_(i j)=1$ ，否则 $m_(i j)=0$ 。
...
所谓的*逆关系*，就是将关系 $R$ 中的每一个元素 $ chevron.l a,b chevron.r$ 都替换为 $ chevron.l b,a chevron.r$ ，记为 $R^(-1)$ ，即：

$
R^(-1)= { chevron.l b,a chevron.r | chevron.l a,b chevron.r in R }
$

显然，对一个关系逆运算两次可以回到自身，即 $(R^(-1))^(-1)=R$ 。

并且，*逆运算保持子集关系*，即假设关系 $R,S subset.eq A times B$ ，若 $R subset.eq S$ ，则 $R^(-1) subset.eq S^(-1)$ 。
...
假设两个关系 $R subset.eq A times B,S subset.eq B times C$ ，显然我们能通过 $R,S$ 得到一个新的关系使其是 $A$ 到 $C$ 的关系、剔除掉中间的集合 $B$ 。称这种运算为*复合*，记为 $S circle.small R$ ：

$
S circle.small R= { chevron.l a,c chevron.r | exists b in B (chevron.l a,b chevron.r in R and chevron.l b,c chevron.r in S) }
$

>按照常规的思维来说，既然是取 $R$ 的第一个元素、 $S$ 的第二个元素，应该记作 $R circle.small S$ 才对。因此请注意这里关系复合的记法与常规思维的不同。

关系复合*不满足交换律*，但*满足结合律*，即 $T circle.small(S circle.small R)=(T circle.small S) circle.small R$ 。

关系复合还*保持子集关系*。假设有四个关系 $R,S,U,W$ ，且 $R subset.eq S,U subset.eq W$ ，那么 $U circle.small R subset.eq W circle.small S$ 。

同时，如果 $R subset.eq A times B$ ，那么 $ Delta_B circle.small R=R=R circle.small  Delta_A$ 。另外：

$
(S circle.small R)^(-1)=R^(-1) circle.small S^(-1)
$
...
$
 cases(
    T circle.small(R inter S) subset.eq T circle.small R,
    T circle.small(R inter S) subset.eq T circle.small S,
    (R inter S) circle.small T subset.eq R circle.small T,
    (R inter S) circle.small T subset.eq S circle.small T,
) quad ==>
$
...
假设有两个关系 $R,S$ ，分别对应关系矩阵 $ bold(M)_R, bold(M)_S$ ，这两个矩阵中的元素分别记为 $r_(i j),s_(i j)$ 。

(1) $R union S$ 

只需将两个关系矩阵做*逻辑或*运算即可，即 $ bold(M)_(R union S)= bold(M)_R or bold(M)_S=[r_(i j) or s_(i j)]$ ，即将两个矩阵对应位置的元素作或运算即可。

(2) $R inter S$ 

只需将两个关系矩阵做*逻辑与*运算即可，即 $ bold(M)_(R inter S)= bold(M)_R and bold(M)_S=[r_(i j) and s_(i j)]$ ，即将两个矩阵对应位置的元素作与运算即可。

(3) $R-S$ 

 $R-S$ 的关系矩阵使用*特殊的减法*运算： $ bold(M)_(R-S)= bold(M)_R minus.o bold(M)_S=[r_(i j) minus.o s_(i j)]$ ，这个减法定义为只有 $1$ 减去 $0$ 才等于 $1$ 。即：

$
0 minus.o 0=0, quad 0 minus.o 1=0, quad 1 minus.o 1=0, quad 1 minus.o 0=1
$

(4) $R^(-1)$ 

逆关系的关系矩阵就是原关系矩阵的*转置*，即 $ bold(M)_(R^(-1))= bold(M)_R^T=[r_(j i)]$ 。

(5) $S circle.small R$ 

关系复合是将两个矩阵作*逻辑积*运算： $ bold(M)_(S circle.small R)= bold(M)_R\odot bold(M)_S$ 。所谓矩阵逻辑积，实际就是普通的矩阵乘法，但是将数乘换做逻辑与、加和换做逻辑或。

#html.hr()

= 三、关系的性质

接下来我们总假设关系 $R$ 是*一个集合* $A$ 上的关系，即 $R subset.eq A times A$ 。

== 1.自反性

如果对于任意 $a in A$ ，都存在 $ chevron.l a,a chevron.r in R$ ，那么就说 $R$ 是*自反的*。反之，如果都不存在 $ chevron.l a,a chevron.r in R$ ，则说是*反自反的*。
...
显然，自反与反自反与*恒等关系*间存在联系。若一个关系是*自反的*，那么恒等关系必是它的*子集*，即 $ Delta_A subset.eq R$ ；如果一个关系是*反自反的*，那么恒等关系与这个关系*没有交集*，即 $ Delta_A inter R= diameter$ 。
...
从集合的角度而言，如果 $R=R^(-1)$ 则关系是*对称的*，这对应上述的转置矩阵与原矩阵相等；而如果关系与其逆关系的交集是*恒等关系的子集*，即 $R inter R^(-1) subset.eq Delta_A$ ，即可判定它是*反对称的*。
...
还是假定关系 $R$ 是*一个集合* $A$ 上的关系，即 $R subset.eq A times A$ 。这个关系可能不自反、或者不对称、或者不传递，但我们可以通过一些手段*扩展*这个关系(也就是添加一些有序对元素)、并且只添加*最少量*的元素，使之成为一个自反或对称或传递的关系。这个新生成的关系称为*闭包*。
...
自反闭包*保持子集关系*，即若 $R subset.eq S$ ，那么 $r(R) subset.eq r(S)$ 。

自反闭包*与集合的交与并是可交换的*，即：

$
r(R inter S)=r(R) inter r(S)
$

$
r(R union S)=r(R) union r(S)
$
...
对称闭包*保持子集关系*，即若 $R subset.eq S$ ，那么 $s(R) subset.eq s(S)$ 。

自反闭包*与集合并是可交换的*，但*与集合交不是*，即：

$
s(R inter S) subset.eq s(R) inter s(S)
$

$
s(R union S)=s(R) union s(S)
$
...
传递闭包*保持子集关系*，即若 $R subset.eq S$ ，那么 $t(R) subset.eq t(S)$ 。

传递闭包与集合的交并*只有子集的关系*：

$
t(R inter S) subset.eq t(R) inter t(S)
$

$
t(R) union t(S) subset.eq t(R union S)
$

注意到到这里为止我们还没讲如何从原关系生成传递闭包。为此，我们需要先引入*关系的幂*。关系的 $n$ 次幂，记作 $R^n$ ，是关系与自己的 $n$ 次复合。例如， $R^3=R circle.small R circle.small R$ 。

从关系矩阵的角度来说，取 $R^n$ 的关系矩阵中的某一个元素 $m_(i j)$ 。若 $m_(i j)=1$ ，那么这说明元素 $i$ 到 $j$ 之间具有一条*长度为 $n$ 的通路*。例如，原关系的关系矩阵就是在描述哪些元素间具有*直接通路*，也就是长度为 $1$ 的通路。
...
$
t(R)=  union.big_(n=1)^(+ infinity)R^n=R^*
$
...
$
t(R)=  union.big_(k=1)^(n)R^k
$

== 4.Warshall算法

Warshall算法实际就是图论中的Floyd算法。这个算法是利用递推式，从 $ bold(W)_0= bold(M)_R$ 开始，计算到 $ bold(W)_n$ 为止。 $ bold(W)_n$ 本身就是传递闭包结果，这省略了求广义并的过程。

记 $w_(i j)^k$ 为矩阵 $ bold(W)_k$ 的第 $i$ 行 $j$ 列元素。第 $k$ 个矩阵由上一个矩阵，也就是第 $k-1$ 个矩阵完全确定。递推式是：

$
w_(i j)^k=w_(i j)^(k-1) or (w_(i k)^(k-1) and w_(k j)^(k-1))
$
...
接下来假定等价关系 $R subset.eq A times A$ 。
...
$
[a]_R=[b]_R quad "或" quad [a]_R inter [b]_R= diameter
$
...
如果一个关系*自反、反对称、传递*，那么称它为*偏序关系*，也简称为*偏序*，此时我们可以用一个类似小于等于的符号 $ prec.eq$ 来表示它。

在此基础上，如果它是*反自反的*，那么称之为*严格序*关系。事实上，如果一个关系*既反自反又传递*，那么它必是一个*反对称关系*。

如果一个关系*自反且传递、但不反对称*，那么称它为*拟序*关系。

接下来我们着重于偏序关系 $R subset.eq A times A$ ，或者写成 $ prec.eq subset.eq A times A$ ：
...
“界”是针对偏序集 $(A, prec.eq)$ 的*子集*来说的。从偏序集中取一子集 $S$ ，如果 $A$ 中存在一个元素 $a$ 大于等于 $S$ 中的*所有*元素(注意是所有！)，则称 $a$ 是 $S$ 的*上界*；反之，若元素 $a$ 小于等于 $S$ 中的*所有*元素，则称 $a$ 是 $S$ 的*下界*。注意，这里只要求 $a in A$ ，也就是 $S$ 中的元素也可以是 $S$ 的上界或下界。
