...
#tufted.definition[谓词][
对个体进行性质或关系的判断，称为*谓词*。谓词是从个体变量到真值的映射，因而具有“函数”的意味。我们通常用大写字母 $P,Q,R...$ 表示谓词，并在其后的括号中填入个体变量。例如 $P(x)$ 表示“ $x$ 具有性质 $P$ ”。
]

谓词必须作用于个体变量上，但个体变量的数量不限。这样，如果要表示“ $x,y$ 具有关系 $Q$ ”，就可以形式化地表示为 $Q(x,y)$ 。一个谓词能作用的个体数目称为*元数*，元数为 $n$ 的谓词称为 $n$ *元谓词*。

#tufted.remark[一阶与高阶][
谓词也可以作用在谓词、函数、公式之类之上，但这不属于*一阶逻辑*的范畴。一阶谓词只作用在个体上；可以作用在谓词等对象上的谓词称为高阶谓词。
]
...
具体应用时，常常要说明是否是全体个体都符合谓词，这就引入了*量词*。其一是*全称量词*，以 $ forall$ 表示。例如，要表示“全体的 $x$ 都有性质 $P$ ”，就符号化为 $ forall x P(x)$ 。这种命题称为*全称命题*。
...
假设有公式 $A= forall x B$ ，那么个体变量 $x$ 称为量词 $ forall x$ 的*指示变量*，称子公式 $B$ 为这个量词的*辖域*或*作用域*。
...
在上述的例子中，将全称量词 $ forall$ 替换为存在量词 $ exists$ ，也是一样的。
...
*前束范式*是一种结构规整的公式。假设以 $Q$ 表示量词符号即 $ forall$ 或 $ exists$ ， $A$ 表示一个不含*量词*的公式，那么前束范式的形式为：

$
Q_1 x_1 Q_2 x_2 ... Q_k x_k A
$
...
在上述定义中，个体变量指派函数是将一个个体变量指派为论域中的一个个体常量。 $ sigma$ 函数中也可以对一个公式起作用，此时它就类似于命题逻辑中的真值赋值函数。即， $ sigma(A)$ 只能等于 $bold(0)$ 或 $bold(1)$ 。
...
(10)*全称例化*规则，又称为*全称量词消除*规则。这条规则包含两种形式。其一，若 $x$ 不在 $A(x)$ 中的任意量词 $ forall y$ 或 $ exists y$ 的辖域中自由出现，那么：
...
应当注意*全称、存在例化*中前提公式的形式。 $ forall x A(x)$ ， $ exists x A(x)$ 都是*前束范式*，在使用这两个规则之前必须首先将公式化为前束范式。
