...
这里我们直接略去无向图、有向图、顶点集、边集、关联、端点、混合图、(非)标定图、关联、邻接等的概念。但需要知道，通常将一幅*图*记为 $G$ ，以 $V(G),E(G)$ 表示它的*顶点集*和*边集*， $abs(V(G)),abs(E(G))$ 表示它的*顶点数*和*边数*。

假如 $abs(V(G))=n$ ，也就是图有 $n$ 个顶点，那么称之为 $n$ *阶图*。如果 $abs(E(G))=0$ ，也就是图中没有边，那么称之为*零图*。同样地，记 $n$ *阶零图*为 $N_n$ ，它是有 $n$ 个顶点的零图，这其中 $N_1$ 称为*平凡图*。
...
#tufted.theorem[握手定理][
对于一条有向边，有始必有终，因此全图入度之和与出度之和相等；而无向图中的每条边关联两个顶点，因此所有顶点的度数之和等于边数的两倍：

(1)对无向图： $  sum d(v)=2abs(E)$ ；\
(2)对有向图： $sum d^-(v)=sum d^+(v)=abs(E)$。
]

#tufted.corollary[奇度顶点定理][
称度数为奇数的顶点为*奇度顶点*，度数为偶数的顶点为*偶度顶点*。由于总度数之和必为偶数，因此*奇度顶点的个数必然是偶数*。
]

遍历所有顶点的度数。我们将图 $G$ 的*最小度*记为 $ delta(G)=min {d(v) }$ ，它是所有顶点度数中的最小值；将*最大度*记为 $ Delta(G)=max {d(v) }$ ，它是所有顶点度数中的最大值。

那么，若 $G$ 是*简单图*，那就必有 $ Delta(G) <= n-1$ 。
...
给定图 $G=(V,E),G'=(V',E')$ 。假如 $V' subset.eq V,E' subset.eq E$ ，那么称 $G'$ 是 $G$ 的*子图*。其中，假如 $V=V'$ ，那么就称 $G'$ 是 $G$ 的*生成子图*。
...
首先假设图 $G=(V,E)$ ，那么它的一条*通路* $ italic(Gamma)=v_0 e_1 v_1 e_2 ... e_n v_n$ ，是从通路*端点* $v_0$ 到 $v_n$ 的经过边 $e_1,e_2,...,e_n$ 的一条路，其*长度*为 $n$ 。假如 $v_0=v_n$ ，那么称此通路为*回路*。
...
假设图 $G=(V,E)$ 是无向图。如果顶点 $u,v$ 间存在通路，就称 $u$ 和 $v$ 是*可达的*，并记为 $u <-> v$ 。约定顶点与顶点本身总是可达的。

若在图中任意两个顶点都是可达的，那么称 $G$ 是*连通无向图*。当图 $G$ 是*简单图*、并且是*连通图*时总存在 $abs(E) >=abs(V)-1$ 。

如果无向图中存在“独立”的几部分，也就是这几部分之间不存在边将其连通，则称这几个部分(是 $G$ 的子图)为 $G$ 的*连通分支*。我们将 $G$ 的*连通分支数*记为 $p(G)$ 。

如果在图 $G$ 中存在某条边 $e$ ，将其删去之后 $p(G)$ 发生变化#footnote[应该说，必然是增大]，也就是产生了将原来的连通分支分为两个连通分支，则称这条边 $e$ 为 $G$ 的*割边*，或称为*桥*。

== 3.有向图的连通

假设图 $G=(V,E)$ 是*有向图*。如果从顶点 $u$ 到 $v$ 间存在通路，就称 $u$ 可达 $v$ ，并记为 $u -> v$ 。若在 $u -> v$ 的同时有 $v -> u$ ，那么就记 $u <-> v$ 。也同样约定总有 $u <-> u$ 。

如果 $G$ 中任意两个顶点 $u,v$ 中都存在 $u <-> v$ ，那么称 $G$ 为*强连通有向图*；而如果 $u -> v$ 和 $v -> u$ 中必有其一成立，那么称 $G$ 为*单向连通有向图*；如果 $G$ 的基图#footnote[有向图的*基图*，是指将所有有向边都视作无向边之后生成的无向图。]是*无向连通图*，那么称 $G$ 为*弱连通有向图*。

为了判定有向图 $G$ 是强连通的，只需要在图中寻找一条经过每个顶点至少一次的有向回路。因此，强连通有向图是单向连通有向图；而单向连通有向图也是弱连通有向图。

== 4.领接矩阵

*邻接矩阵*在#link("../关系")[关系]中已经介绍过，不再赘述。显然无向图的邻接矩阵必是*对称矩阵*，而有向图则不一定。有向图的领接矩阵中，*行*中元素的和是此行对应顶点的*出度*，而*列*中元素的和是此列对应顶点的*入度*。简单图的邻接矩阵中元素只有 $0,1$ 两种。

对于图的邻接矩阵 $A$ ，以普通的乘法来计算它的幂，那么 $A^k$ 就是表明两个顶点间长度为 $k$ 的通路的条数；因此以#link("../关系")[关系]中的特殊乘法来说，就只是判断是否存在长度为 $k$ 的通路。

而记 $B=A+A^2+...+A^(n-1)$ ，这个矩阵表明两个顶点间长度小于等于 $n-1$ 的通路的条数。这样，将这个矩阵中的元素单位化，形成一个只有 $0,1$ 的矩阵 $P$ ，这就是图 $G$ 的*可达矩阵*，相当于#link("../关系")[关系]中的传递闭包。因此可达矩阵同样可以使用 $"Warshall"$ 算法求出。
...
(3) $G$ 中没有回路，并且 $abs(E)=abs(V)-1$ ；\
(4) $G$ 是连通的，并且 $abs(E)=abs(V)-1$ 。
]

树中的等式 $abs(E)=abs(V)-1$ 与握手定理结合，常常是证明的要点。
...
与握手定理同样的原理，一条边必是两个面的边界，因此*平面图的所有面的度数之和等于边数的两倍*： $  sum deg(R)=2abs(E)$ 

第二个重要的公式是*欧拉公式*。记连通平面图有 $n$ 个顶点、 $m$ 条边、 $d$ 个面，那么恒有 $n-m+d=2$ 。

记 $g(G)$ 是图 $G$ 中*长度最小的回路*的长度，称为 $G$ 的*围长*。在平面图中，围长显然就是所有面中最小的度数。记 $l=g(G)$ ，那么由第一条定理，应有 $l times d <= 2m$ ；而由欧拉公式，应有 $d=2+m-n$ 。联合两式，得到：
...
对于无向图中的一条通路 $ italic(Gamma)=v_0 e_1 v_1 e_2 ... e_n v_n$ ，如果不存在其他顶点 $v in V,v != v_i$ 使 $v$ 与 $v_0$ 或 $v_n$ 相邻，那么称此通路为*极长通路*。如果此极长通路又是*初级通路*，那么称之为*极长初级通路*。

一个简单图存在*哈密顿通路*的充分条件是：如果 $n$ 阶简单图 $G$ 中的任意两个顶点的度数之和都大于等于 $n-1$ ，那么 $G$ 存在*哈密顿通路*。这里要求 $n >=2$ 。

因此，若 $n(n >=3)$ 阶的简单图 $G$ 中的任意两个顶点的度数之和都大于等于 $n$ ，那么 $G$ 存在*哈密顿回路*；或者，任意顶点的度数都大于等于 $ frac(n, 2)$ ，那么 $G$ 存在*哈密顿回路*。
