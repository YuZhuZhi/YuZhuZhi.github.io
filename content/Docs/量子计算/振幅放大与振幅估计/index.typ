...
注意到 $a=sin^2 theta$ ，现在问题转化为估计 $ theta$ 。注意到算符 $ upright(bold(Q))$ 及其作用次数使得好纯态与坏纯态的振幅形成了三角函数，因此估计这些三角函数的最大周期 $ frac(pi, theta)$ 亦可。另一方面，注意到算符 $ upright(bold(Q))$ 的*特征根*是 $te^(plus.minus ti 2 theta)$ ，利用#link("../量子傅里叶变换与相位估计/")[量子计算（六）——量子傅里叶变换与相位估计]中的*相位估计*来获得 $ upright(bold(Q))$ 的特征根也是一种可行的方法。
