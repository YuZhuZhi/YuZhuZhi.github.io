...
// bHC ket(x) =(sum_(delta) h_(delta)times.o.big_(i = 1)^(n)bold(Z)_(i)^(delta_(i))) ket(x) =(sum_(delta) h_(delta)product_(i = 1)^(n)s_(i)^(delta_(i))) ket(x) = cal(H)(s) ket(x).
// $
// 在不忽略常数项的情况下，$cal(H)(s) = cal(C)(x)$，所以$bHC ket(x) = cal(C)(x) ket(x)$，故$ket(x)$对应的特征值就是相应的代价$cal(C)(x)$。  #h(1fr) $square.stroked$
...
/* End subequations */
 这说明混合酉算子$U(bHB, beta)$的作用就是在每一个量子比特上都作用一个$R X(2 beta)$门，如图@fig-chap9-MixerUnitaryOperatorEffect 所示。$R X(theta)$门是： $  R X(theta) = mat(delim: "[", cos theta/2, - upright(i) sin theta/2 ; - upright(i) sin theta/2, cos theta/2)  $
...
*Theorem 3.* $Z_(i_(1))$与$Z_(i_(1))times.o Z_(i_(2))$对应的酉算子及其量子电路 <theo-chap9-SingleCoupleFoldZGate-QuantumCircuit> 
...
_Proof._  首先当$bold(cal(H)) = bold(Z) $时做如下计算： /* Begin subequations */
$
...
/* End subequations */
 在考虑将要作用到的量子比特后定理第二部分得证。  #h(1fr) $square.stroked$

必须指出，图
// #ref(<fig-chap9-SingleCoupleFoldZGate-QuantumCircuit>)
中的$q_(i_(1))$与$q_(i_(2))$并不一定是相邻的，$i_(1),i_(2)$的取值只取决于式@eq-chap9-QuestionHamiltonian 中的$delta $。由此定理得到以下推论：

*Corollary 1.* $times.o.big_(k = 1)^(n) Z_(i_(k))$对应的酉算子及其量子电路 <coro-chap9-MultiFoldZGate-QuantumCircuit> 
...
_Proof._  证明留给读者作为习题。提示：使用数学归纳法。  #h(1fr) $square.stroked$
...
*Theorem 4.* _伊辛哈密顿量的缩放 <theo-chap9-伊辛哈密顿量的缩放> 
设缩放参数$k > 0 $。若自旋构型$s^(*)$是被缩放后的伊辛哈密顿量$cal(H)(s)/k $的最优解，那么$s^(*)$同样是原始伊辛哈密顿量$cal(H)(s)$的最优解。 _
...
*Theorem 5.* _QAOA算法的能量景观中$bold(beta) $参数的周期性 <theo-chap9-QAOA算法的能量景观中beta参数的周期性> 
在忽略产生的全局相位的情况下，QAOA算法的能量景观的参数$bold(beta) $具有周期$pi $，即： $  lr(chevron.l bHC chevron.r)bold(gamma), beta_(1), ..., beta_(i)+ pi, ..., beta_(p)) = lr(chevron.l bHC chevron.r)bgabe), quad i = 1,2,...,p  $ <eq-chap9-QAOAEnergyLandscapeBetaPeriodicity>
 _

_Proof._  注意到由式@eq-chap9-QAOAUnitaryOperatorDefine 代入混合哈密顿量推导出的图@fig-chap9-MixerUnitaryOperatorEffect 中 RX 门的角度参数均为$2 beta $，因此$pi $是能量景观关于$bold(beta) $参数的最小正周期。  #h(1fr) $square.stroked$
...
*Theorem 6.* QAOA算法的能量景观中$bold(gamma) $参数的周期性 <theo-chap9-QAOA算法的能量景观中gamma参数的周期性> 在忽略产生的全局相位的情况下，形如式@eq-chap9-IsingHamiltonianPolynomial 的伊辛哈密顿量中：
...
_Proof._  存在无理数系数的情况下：待证。

所有系数均为有理数的情况下：由定理
// #ref(<theo-chap9-SingleCoupleFoldZGate-QuantumCircuit>)和推论#ref(<coro-chap9-MultiFoldZGate-QuantumCircuit>)，
考虑相关系数后即按照式@eq-chap9-QuestionUnitaryOperatorEffectWithCoeffientB， RZ 门的角度参数是$2 h_(delta)gamma $。故对单个$gamma $，其周期是$frac(pi, h_(delta))$。那么对于所有$gamma $、即$bold(gamma) $具有最小公共正周期$"lcm"(frac(pi, h_(delta))) = frac(pi, gcd(h_(delta)))$。  #h(1fr) $square.stroked$
...
*Theorem 7.* _QAOA算法的能量景观的对称性 @giovagnoli-2025-An-Introduction-to-the-Quantum-Approximate-Optimization-Algorithm <theo-chap9-QAOA算法的能量景观的对称性> 
...
*Theorem 8.* _QUBO问题特殊情形下对应QAOA算法的能量景观的周期性 @giovagnoli-2025-An-Introduction-to-the-Quantum-Approximate-Optimization-Algorithm <theo-chap9-QUBO问题特殊情形下对应QAOA算法的能量景观的周期性> 
...
_Proof._  注意到对任一层酉算子有： /* Begin subequations */
$
...
是因为初态$ket(s)$就是$product_(i = 1)^(n)bold(X)_(i)$的特征向量。因此，在忽略全局相位的情况下能量景观也是不变的。  #h(1fr) $square.stroked$

// 进一步地，将定理@theo-chap9-QUBO问题特殊情形下对应QAOA算法的能量景观的周期性推广至PUBO问题得到以下推论。

*Corollary 2.* _特殊情形下QAOA算法的能量景观的周期性 @giovagnoli-2025-An-Introduction-to-the-Quantum-Approximate-Optimization-Algorithm <theo-chap9-特殊情形下QAOA算法的能量景观的周期性> 
...
_Proof._  该推论为读者留作习题。  #h(1fr) $square.stroked$
