...
即使哈希函数分布理想，当元素越来越多时，碰撞仍然必然增加。衡量表“拥挤程度”的量是负载因子（load factor）$alpha=n/m$，其中 $n$ 为元素数、$m$ 为桶或可用槽位数。链地址法允许多个元素留在同一桶，所以 $alpha$ 可以超过 $1$，但平均桶长会随之增长；开放地址法把所有元素放在固定槽位数组中，必须保留空槽作为查询终止标志，因而 $alpha$ 必须小于 $1$。
...
#tufted.definition[重哈希（rehash）][申请更大的表，并按新容量重新计算每个元素的位置。旧表中的桶下标依赖容量，不能简单把数组尾部补空。重哈希耗时 $O(n)$，但按倍数增长时，插入仍可得到摊还 $O(1)$。]
...
链地址法（chaining）让每个桶保存一个元素集合，传统教材把这个集合画成链表。查询先计算桶下标，再在桶内逐个比较。实现常把完整哈希值与键一同保存：先比较整数哈希，只有相同才调用可能更昂贵的键相等比较。
...
开放地址法（open addressing）不为每个冲突元素另外分配节点，而是把所有元素直接放进槽位数组。键的初始位置被占用后，按照预先规定的探测序列继续寻找。只要插入与查询使用相同序列，查询就能重走插入路径并找到目标。
