...
*线性表*在逻辑上看似只是“把元素排成一排”，但“一排”只说明元素之间的逻辑关系，并没有规定元素在内存中应当怎样放置。如果从内存的角度观察，我们既可以把它们依次放进一段连续内存得到顺序表；也可以让每个元素各自占据一块内存、再用指针连接前后关系，得到链表。而从访问方式的角度来看，还可以限制数据只能从某一端输入与访问，于是得到栈；或者规定数据从一端进入、另一端离开，于是得到队列。
