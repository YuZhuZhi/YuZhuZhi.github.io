...
线性结构看似只是“把元素排成一列”，但“一列”只说明元素之间的逻辑关系，并没有规定元素在内存中应当怎样放置。我们既可以把它们依次放进一段连续内存，也可以让每个元素各自占据一块内存、再用指针连接前后关系；还可以限制只能从某一端访问，于是得到栈，或者规定从一端进入、另一端离开，于是得到队列。
...
顺序表是线性表的一种*存储实现*：逻辑上相邻的元素，在物理内存中也相邻。假如第一个元素地址为 $p$，每个元素占据 $s$ 字节，那么第 $i$ 个元素的地址就是 $p+i s$。正因地址能由一次乘加直接算出，按下标访问不需要从头遍历，复杂度为 $O(1)$。

长度在编译期固定时，可以使用数组 `[T; N]`。然而实际程序往往无法预先知道元素数，因此更常用可增长数组 `Vec<T>`。一个 `Vec` 的值本身只保存三个字段：`ptr` 指向堆上的缓冲区，`len` 表示其中已经初始化的元素数，`capacity` 表示当前缓冲区最多容纳多少个元素。必须区分后两者：`len..capacity` 对应的内存已经申请，却还没有放入有效的 `T`，不能被当作元素读取。
...
#tufted.definition[摊还复杂度][一次扩容需要搬运 $n$ 个元素，是 $O(n)$；若容量按常数倍增长，连续 $n$ 次 `push` 的总搬运量是几何级数 $O(n)$，故单次 `push` 的摊还复杂度为 $O(1)$。]

当 `push` 发现 `len == capacity` 时，向量需要申请更大的缓冲区，并把旧元素移动过去。这里说“移动”而不是“复制”十分重要：`T` 可以是 `String`、文件句柄或其他不能随意复制的值，Rust 只转移它们的所有权，不会凭空制造第二份资源。扩容完成后，旧缓冲区被释放，原先指向其中元素的引用也就不再有效；借用检查器因此不允许我们在持有 `&vec[i]` 时再调用可能扩容的 `push`。

下面的教学实现刻意把底层内存管理交给 `Vec<T>`，只实现“顺序表”这一层语义。这样既能看到接口和复杂度，也不会把主题淹没在分配器、对齐与 `MaybeUninit<T>` 等底层细节中：
...
`get` 返回 `Option<&T>` 而不是直接索引，使“可能越界”成为返回类型的一部分；调用者必须处理 `None`。`get_mut(&mut self)` 的完整类型可以写成 `fn get_mut<'a>(&'a mut self, ...) -> Option<&'a mut T>`：返回引用的生命周期来自对顺序表的借用，不可能比顺序表本身活得更久。在这个可变引用存活期间，调用者也不能再次借用整个表去执行插入或删除。插入可能扩容，删除会搬移元素，若允许这些行为发生，旧引用就可能立即悬空或指向另一个元素。

中间插入时，`Vec::insert(i, value)` 会把区间 `[i, len)` 整体向后移动一个槽位；删除时则把 `(i, len)` 向前移动。两者都必须特别处理 `i == len`、空表以及越界位置。若只关心快速删除而不要求保持顺序，可以使用 `swap_remove`：它把最后一个元素移到删除位置，从而把 $O(n)$ 降为 $O(1)$，代价是改变元素次序。这个例子说明复杂度不是脱离接口的标签——“删除”究竟需不需要保持相对次序，直接决定可用的算法。

顺序表按下标访问为 $O(1)$；尾部追加摊还 $O(1)$；中间插入、删除要移动后缀，均为 $O(n)$。然而复杂度只统计操作次数，没有反映现代处理器一次缓存未命中的巨大代价。连续内存允许硬件预取器提前加载后续元素，也使一次缓存行能够带回多个值；所以即使顺序表在理论上需要移动若干元素，它在真实机器上仍经常胜过只改几条指针的链表。工程上应先选 `Vec<T>`，再用测量结果证明自己确实需要其他布局。

#html.hr()

= 二、链表

链表把“相邻”从物理地址关系改成指针关系。每个节点除保存元素外，还保存指向其他节点的连接；节点可以分散在堆上，所以插入时不需要搬动后续元素。但这句话常被误解为“链表插入总是 $O(1)$”：只有在*已经持有插入位置的节点句柄*时才是如此；若必须先按下标寻找第 $i$ 个节点，查找本身仍需 $O(n)$。

链表还付出了额外空间与访问延迟。单链表每个节点多一个指针，双链表多两个；分配器又可能为每块内存附加元数据和对齐空隙。遍历时，处理器必须先读出当前节点中的地址，才能知道下一次应访问哪里，这种数据依赖使预取更加困难。理解这些代价之后，我们才能判断“修改指针次数少”是否真的意味着程序更快。
...
以下都以双链表为语义模型。它在代码中维护的不只是若干指针，而是一组必须始终同时成立的不变量：空表的 `head` 与 `tail` 同时为空；非空表从 `head` 沿 `next` 恰好访问 `len` 个节点并到达 `tail`；首节点没有前驱，尾节点没有后继；任一相邻节点 `a`、`b` 同时满足 `a.next == b` 与 `b.prev == a`。所谓边界条件，本质上就是某次修改是否仍保持这些不变量。
...
快慢指针并不是某个固定算法，而是一种在同一条链上维持两个不同移动速度或不同间距的技巧。链表不能用下标直接跳到目标位置，但两个指针之间的*相对运动*仍能编码长度与距离信息，并且只需要 $O(1)$ 额外空间。

最直接的用途是寻找单链表中点。令 `slow` 与 `fast` 都从首节点出发，`slow` 每轮前进一步，`fast` 每轮前进两步。当 `fast` 到达链尾时，它走过的距离约是 `slow` 的两倍，因此 `slow` 恰好位于中间。奇数长度链表只有一个中点；偶数长度链表有两个候选中点，下面的停止条件返回*后一个*中点。若希望返回前一个中点，可以在循环前进一步检查 `fast.next.next` 是否存在。
...
相同思想还能寻找倒数第 $k$ 个节点：先让 `fast` 前进 $k$ 步，再让两个指针同速前进；`fast` 到达链尾时，`slow` 与链尾恰好相距 $k$ 个节点。若预先前进不足 $k$ 步就遇到链尾，说明 $k$ 超过链表长度。相比先遍历一次求长度再遍历第二次，这种方法只需一次完整扫描。
...
链表仍适合侵入式调度队列、内核对象表、内存分配器的空闲块、LRU 中“哈希表 + 双链表”的顺序维护，以及要求节点地址稳定的场景。然而现代 CPU 以缓存行为为中心：链式节点分散、指针追逐难以预取，额外指针放大内存占用，逐节点分配也昂贵。于是大量业务代码用 `Vec`、`VecDeque`、紧凑 B 树或“数组 + 索引”替代传统链表。

#html.hr()

= 三、栈

栈的抽象只有后进先出：最后压入的元素最先弹出。它至少提供 `push`、`pop` 和 `peek` 三个操作，至于是连续数组还是链表，是实现细节。表达式求值、括号匹配、深度优先搜索与撤销操作看似不同，实质上都需要保存“尚未处理完、稍后按相反顺序恢复”的状态。
...
这正是“接口与实现分离”的意义：调用者依赖后进先出的行为契约，而不是依赖 `Vec` 的某个具体字段。只要新实现仍满足同一契约，库作者便可以在不改业务逻辑的前提下替换内存布局、加入容量限制或记录统计信息。
...
队列提供先进先出的 `enqueue` 与 `dequeue`：新元素从队尾进入，旧元素从队首离开。操作系统的就绪任务、网络数据包和图的广度优先遍历都具有这种“先到先处理”的语义。若用普通数组每次删除第一个元素，就必须整体搬移后续元素而成为 $O(n)$；正确做法是让逻辑头下标向前移动，而不是移动所有元素。
...
优先队列取出的不是最早进入者，而是优先级最高者。Rust 的 `BinaryHeap` 是最大二叉堆：查看堆顶 $O(1)$、插入和弹出 $O(log n)$。它将在《树与堆》中完整实现；此处只需记住，“队列”描述接口顺序，“堆”描述一种实现。
...
#table(
  columns: (0.8fr, 1.25fr, 1.2fr, 2.25fr),
  table.header([语言], [常用队列类型], [实际布局], [含义]),
  [Rust], [`VecDeque<T>`], [可增长环形缓冲区], [元素存放在数组式缓冲区中，首尾下标可以回绕；逻辑序列因此可能分成缓冲区末尾与开头两段。两端插删为摊还 $O(1)$。],
  [Java], [`ArrayDeque<E>`], [可增长环形数组], [它属于环形顺序存储而不是链表：队首、队尾下标在数组中回绕，容量不足时再扩容。普通单线程 FIFO 通常用它。],
  [Java], [`ArrayBlockingQueue<E>`], [定长环形数组], [容量创建后不变；队首、队尾循环复用槽位，并在空或满时让消费者或生产者等待，适合生产者—消费者模型。],
  [Java], [`LinkedList<E>`、`LinkedBlockingQueue<E>`], [链式], [前者以双向链表实现 `Deque`，后者以链节点组成可选有界的阻塞队列；它们承担节点分配与指针开销。],
  [C++], [`std::queue<T>`], [容器适配器，默认底层为 `std::deque<T>`], [`queue` 本身不规定单独的存储结构，只把底层容器的 `push_back` 与 `pop_front` 暴露为 FIFO 接口；也可显式改用 `std::list<T>`。标准只规定 `deque` 的接口与复杂度，常见实现采用分块顺序存储，但具体布局属于实现细节。],
)
