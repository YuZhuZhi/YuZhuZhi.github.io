...
#tufted.definition[邻接矩阵（adjacency matrix）][给顶点编号 $0,1,dots,n-1$。图 $G$ 的邻接矩阵是 $n times n$ 矩阵 $A$：无权图令 $A_(u,v)$ 表示边是否存在；带权图令其保存边权。若图允许零权边，必须另外使用 `Option<W>` 或无穷哨兵表示“无边”，不能把数值 0 同时承担两种语义。]
...
邻接表（adjacency list）为每个顶点建立一张出边表。外层数组的下标是顶点编号，`adj[u]` 中的每个记录保存终点与权值；于是遍历 $u$ 的邻居只访问真实存在的 $deg^+(u)$ 条边。对于有向图，总记录数为 $E$；对于无向图，每条边在两端各存一次，总记录数为 $2E$，但空间仍为 $Theta(V+E)$。
...
#tufted.definition[拓扑序（topological order）][有向图 $G=(V,E)$ 的一个拓扑序是顶点的线性排列 $v_(1),v_(2),dots,v_(n)$，满足对每条边 $(u,v) in E$，$u$ 都出现在 $v$ 之前。拓扑序表达偏序约束的一种线性扩展，通常并不唯一。]
