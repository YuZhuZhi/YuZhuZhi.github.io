...
机器整数只有固定宽度，超过范围会溢出；任意精度整数（arbitrary-precision integer）把一个整数拆成若干“肢”并用动态数组保存。真正困难的不只是把数组变长，而是维护唯一表示、符号、进借位、临时空间和不同规模下的算法切换。本章只讨论整数。
...
#tufted.definition[规范表示（canonical form）][本文把符号与绝对值分离：`sign ∈ {-1, 0, 1}`，`limbs` 只保存幅值且按低位在前排列。规范表示要求最高端没有值为 0 的肢，并规定数学上的零唯一写成 `sign = 0, limbs = []`；非零值的符号只能为 `-1` 或 `1`。]
...
#tufted.definition[离散卷积（discrete convolution）][有限序列 $a,b$ 的卷积 $c=a*b$ 定义为 $c_(k)=sum_(i+j=k)a_(i) b_(j)$。大整数肢数组的未进位乘积恰好是这一卷积，所以任何快速多项式卷积算法都可以转化为大整数乘法。]
...
Java 标准库 `java.math.BigInteger` 是不可变任意精度有符号整数。`add`、`multiply` 等方法返回新对象，因此不存在调用者可观察的原地修改；位运算公开表现为无限长二进制补码语义，但实现可以把符号与幅值分开存储。就 `multiply` 而言，实现按内部 `mag` 数组的 `int` 元素数分梯队：少于 `KARATSUBA_THRESHOLD = 80` 时用朴素乘法，80 起改用 Karatsuba，达到 `TOOM_COOK_THRESHOLD = 240` 后再走 Toom-Cook 三路算法（Toom-3）。阈值是当前实现特征而非语言保证，OpenJDK 已把两处值由早期的 50/75 调整为 80/240。#cite(<java-biginteger>)#cite(<openjdk-biginteger>)

Python 的 `int` 本身就是任意精度整数，语法层面不区分机器小整数与大整数。数值增长越过内部范围后仍保持精确，但时间与空间当然会随位数增加；“不会溢出”不等于“运算成本仍为常数”。`int` 的乘法由 CPython 的 `Objects/longobject.c` 实现：两个操作数的内部 `PyLong` 数字都未超过 `KARATSUBA_CUTOFF = 70` 时用朴素乘法，否则进入 Karatsuba；`int` 乘法没有 Toom-Cook 或 FFT 路径。#cite(<cpython-longobject>) 固定宽度二进制协议、哈希截断或与 C 接口交互时，仍必须显式检查范围和字节序。

== 2. Rust 与 C++：通过库选择语义

Rust 标准库只提供固定宽度整数，常用 `num-bigint` 的 `BigUint` 表示非负幅值，`BigInt` 表示有符号整数。运算符 trait 具有拥有值与借用值的多种实现；循环中写 `&a * &b` 可以避免为了传参克隆两个大操作数，但结果缓冲仍需分配。`BigUint::mul` 的底层 `mac3` 按肢数选算法：32 个 `BigDigit` 以内逐肢乘加（朴素乘法），超过 32 后改用 Karatsuba（Toom-2），超过 256 后进入 Toom-3；阈值由 `cargo bench` 基准调出，属于当前实现细节而非接口契约。解析、格式化和字节转换都通过明确方法完成。#cite(<num-bigint>)

C++ 标准库同样没有统一任意精度整数。Boost.Multiprecision 提供与泛型数值代码较易结合的类型，GMP 则是常见的高性能底层库。二者的乘法梯队也不同：Boost 自研后端（如 `cpp_int`）只有朴素乘法与 Karatsuba，官方文档明确表示更高级的 Toom-Cook 与基于 FFT 的乘法尚未实现；GMP 则按机器调好的阈值依次启用朴素乘法、Karatsuba、更高阶 Toom-Cook，直至 Schönhage–Strassen 的 FFT 路径，操作数越大越依赖这一多级切换。#cite(<boost-multiprecision-perf>)#cite(<gmp-algorithms>) 不同后端在异常、内存分配、表达式模板、线程与许可证方面有不同约束；选择“大数库”不仅是比较乘法复杂度，还要看整个项目的接口边界。
