...
形态学图像处理关心的不是“像素值相差多少”，而是一个形状能否容纳、碰到或避开另一个小形状。这个小形状称为结构元素。把结构元素在图像上平移，就能把局部几何关系转化为腐蚀、膨胀、开闭运算、边界、连通性和骨架等可计算对象。#cite(<dip-book>)
...
结构元素不是越大越好。过小的结构元素无法消除目标尺度的噪声；过大的结构元素会抹掉窄连接、尖角和小目标。十字形结构元素偏向四邻接，方形结构元素同时连接对角方向，圆盘近似各向同性。文中先建立二值集合定义，再给出灰度级扩展。
