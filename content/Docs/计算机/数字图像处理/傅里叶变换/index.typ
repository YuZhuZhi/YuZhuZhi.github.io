...
本章首先从一维傅里叶级数过渡到连续傅里叶变换和 DFT，再把结论推广到二维图像，最后解释 FFT 为何能把 DFT 的计算量从平方级降到近线性对数级。这里集中建立后续频域滤波所需的数学与算法。除另加说明，本章统一采用角频率（弧度/单位）约定，虚数单位记作 $ii=upright(i)$，正变换指数为 $-ii omega t$；这一约定在进入二维与频域滤波后不再重复声明。关于广义函数、连续时间系统和采样的更完整讨论，可参阅#link("../../../数学/信号系统/连续时间傅里叶/")[连续时间傅里叶]与#link("../../../数学/信号系统/单位冲激函数与卷积/")[单位冲激函数与卷积]。
...
$W_(N)^k$ 是单位圆上的 $N$ 次单位根。频率索引每增加 $N$，指数就多转整数圈，所以 $X[k+N]=X[k]$；输入也被解释为 $x[n+N]=x[n]$。这正是 DFT 的周期性来源。本章定义的这个 $W_(N)$ 会在二维 DFT、逐次加倍法与蝶形因子中反复出现，是后续所有分析共享的底层对象。
...
$
乘上的复指数模为 1，所以幅度谱完全不变。物体移动后幅度谱看似相同，并不代表图像内容的位置没有改变；位置信息进入了相位。反过来，只保留幅度而丢掉相位会失去位置关系，这正是后面频域滤波讨论相角作用的根据。
