...
分割把图像变成边界、区域或目标掩膜，特征提取则把这些像素集合进一步压缩为便于比较的数值描述。轮廓可以编码为方向序列，角点可以由局部灰度在两个方向上的变化确定，稳定区域可以通过阈值变化追踪，局部纹理则可以组织成具有尺度和方向的描述向量。图像拼接正是这些思想的综合应用：它先在不同图像中找到可重复定位的局部特征，再由匹配关系估计视图之间的几何变换。#cite(<dip-book>)
...
#tufted.definition[角点][在给定观察尺度下，若图像局部窗口沿两个线性无关方向发生微小位移时，灰度匹配误差均显著增加，则窗口中心称为角点。]
...
$
每对点提供两个约束，理论上至少需要四对一般位置的对应点；若四点近乎共线，方程会病态。单应性适合近似平面场景或相机绕光心旋转。若相机发生明显平移且场景有较大深度差，近景和远景产生不同视差，一个单应性无法同时对齐全部像素。
