...
$
  g=cal(H){f}+eta.
$
复原算法接收 $g$ 以及对 $cal(H)$、$eta$ 的估计，输出原图估计 $hat(f)$。同一幅模糊图可能由运动、离焦或大气扰动造成；退化原因不同，逆运算也不同。因此，复原不能从“选哪个锐化滤波器”开始，而应从噪声、点扩散函数和先验约束开始。#cite(<dip-book>)
...
噪声模型描述随机灰度扰动可能取什么值、各值出现的概率以及它是否依赖位置或原图。概率密度不是噪声图像本身：同一分布可以产生许多不同样本，而参数决定其均值、方差、偏斜程度和尾部。
...
设中心化频谱中心为 $(P/2,Q/2)$，第 $k$ 个噪声峰相对中心的偏移为 $(u_(k),v_(k))$。到正、负共轭峰的距离分别为
$
  D_(k)^+(u,v)=sqrt((u-P/2-u_(k))^2+(v-Q/2-v_(k))^2),
$
$
  D_(k)^-(u,v)=sqrt((u-P/2+u_(k))^2+(v-Q/2+v_(k))^2).
$
...
维纳滤波不要求把退化完全反转，而是在噪声放大与去模糊之间最小化均方误差。设使用线性频域估计 $hat(F)=A G$，目标为
$
  min_(A) E[abs(F-A G)^2].
$
代入 $G=H F+N$，假设信号与噪声不相关，展开期望并对 $A^ast$ 求极值，可得正规方程
$
