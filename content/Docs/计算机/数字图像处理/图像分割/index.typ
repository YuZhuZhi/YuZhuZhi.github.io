...
分割结果没有脱离任务的唯一“正确答案”。同一张医学图像可以按器官、病灶或组织纹理分割；阈值、平滑尺度、邻接方式和标记都会改变结果。因此算法说明必须同时给出假设、参数和失败条件。
...
#tufted.definition[边缘][边缘是图像某个局部性质发生显著变化的位置。灰度边缘通常表现为一阶导数的局部极大值或二阶导数的过零点；它是有限宽度采样与模糊下的模型，而不是必然精确落在物体真实边界上的单像素曲线。]
...
只检查相邻像素符号不同会产生大量弱噪声边缘，还应要求过零两侧的响应差超过阈值。二维中通常检查水平、竖直和两条对角方向。LoG 的闭合轮廓能力较好，但二阶导数可能在宽斜坡两侧产生双边缘。
...
Otsu 方法不假定两个峰服从高斯分布，而是在所有候选阈值中选择类间方差最大的一个。设灰度级为 $0,dots,L-1$，直方图计数为 $n_(i)$，总像素数为 $N$，归一化概率为
$
  p_(i)=n_(i)/N,
  quad
  sum_(i=0)^(L-1)p_(i)=1.
