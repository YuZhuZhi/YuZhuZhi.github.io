...
数字图像中的灰度值既决定整体明暗，也决定物体与背景能否分辨。原始图像若动态范围狭窄、暗部挤在少数灰度级，或成像设备的响应与观察需求不一致，就需要重新安排输入与输出灰度的对应关系。设输入灰度为 $r$、输出灰度为 $s$，单像素变换统一写成
$
  s = T(r).
$
映射确定后，同一函数 $T$ 独立作用于每个坐标，因此像素的位置和邻接关系都不改变，改变的是灰度的分布。这一点把本章方法分成两类：反相、对数、伽马与分段线性变换预先给定 $T$；直方图方法先统计整幅图或局部区域，再由统计结果构造映射。后者输出虽仍逐像素查表，映射本身却依赖其他像素，所以不能与预先固定的点运算混为一谈。区分二者的判据不在于“是否逐像素查表”，而在于查表是否与其余像素有关。#cite(<dip-book>)
...
$
  s=c log(1+r).
$
加 1 是为了让 $r=0$ 时有定义。对数曲线在暗部斜率大、亮部斜率小，因此扩展低灰度差异、压缩高灰度动态范围。若输入归一化到 $[0,1]$，取 $c=1/log 2$ 可使输出仍落在 $[0,1]$；若直接处理 $[0,L-1]$，取 $c=(L-1)/log L$。选择哪个常数取决于输入范围，而不是取决于“想提亮多少”。傅里叶频谱幅值常横跨多个数量级，显示时常用 `log1p`，但该显示变换不应覆盖待逆变换的原始频谱。
...
高位平面通常包含主体轮廓和大尺度明暗关系，因为改变高位会造成较大的灰度跳变；低位平面包含细小变化，也更容易混入量化噪声。不过“低位必然是噪声”并不成立：规则纹理、水印或精密测量信息也可能出现在低位。判断某一平面是否有用，应结合成像过程和任务，而不是仅凭位序。下面用一幅真实灰度照片的 8 个比特平面说明这种差异。

#figure(
  image("images/bit-planes-real.png", width: 100%, alt: "真实灰度照片及其八张比特平面的系列图，标注为 a 到 i"),
  caption: [
    以一张真实灰度照片为例，按 $k$ 从高到低逐层展开。面板 (a) 是原图；(b) 到 (i) 依次为第 7 位到第 0 位比特平面（$b_7$ 到 $b_0$），二值平面已按黑白显示（0 为黑、1 为白）。$b_7$ 到 $b_5$ 保留主体轮廓与大尺度明暗，$b_2$ 到 $b_0$ 只剩近乎随机的细微起伏；可见“高位含结构、低位含噪声”只是趋势，究竟哪些位有用取决于图像本身。
  ]
)
