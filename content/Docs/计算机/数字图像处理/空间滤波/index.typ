...
#tufted.theorem[线性滤波的叠加性][固定核和边界规则后，线性空间滤波算子 $cal(H)$ 满足
$
  cal(H)(a f+b q)=a cal(H)(f)+b cal(H)(q).
$
]

#tufted.proof[把 $a f+b q$ 代入加权和，利用乘法对加法的分配律，将包含 $f$ 与 $q$ 的有限求和分开即可。]

叠加性让复杂滤波器可以由简单核相加、相减或串联分析。不过，只有核固定且边界处理一致时，上述结论才直接成立；若权重随图像内容改变，运算一般不再线性。
