...
对可分离核不必一次完成二维加权和。这里的 $a$ 是 $m times 1$ 列向量、$b^T$ 是 $1 times n$ 行向量；先沿一个方向用 $b^T$ 做一维滤波得到中间图像，再沿另一方向用 $a$ 滤波，结果与二维核相同。每个输出像素的乘加次数从约 $m n$ 降为 $m+n$；以 $15 times 15$ 核为例，从 225 项降到 30 项。这也是大尺度高斯滤波常采用分离实现的原因。
...
低通滤波器保留缓慢变化的成分，削弱像素间的快速起伏。在空间域中，它通常表现为邻域平均或稳健汇总，输出比输入平滑。平滑能抑制噪声和细纹理，却同样削弱窄结构与清晰边缘；因此窗口大小的选择是对“压住多少噪声”与“保住多细结构”的权衡，而不是越大越好。决定窗口的依据应是目标尺度与噪声尺度：窗口必须能容纳噪声以充分平均，又不得吞掉你要保留的最小结构。
...
只要已有低通滤波器，就能用相减构造与它互补的高通；再准备两个截止尺度不同的低通，就能进一步构造带通与带阻。因此这里无需从头设计三套互不相关的核。关键的不变式是：对任何由低通算子 $cal(L)$ 得到的图像，$cal(L)(f)$ 与 $f-cal(L)(f)$ 恰好把原信号按“缓慢变化”与“其余变化”拆成互补的两份，二者相加还原 $f$。带通与带阻正是对这两份再作一次拆分。
