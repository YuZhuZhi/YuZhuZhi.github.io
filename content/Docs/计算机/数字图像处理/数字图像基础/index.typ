...
经典成像模型先把场景在像平面上的光学分布视为连续函数，计算机实际保存和处理的则是有限数组。空间采样把连续坐标限制到离散网格，灰度量化再把连续强度限制到有限数值；二者共同决定原始信息如何进入数字系统。本章先建立数字图像的矩阵模型，再讨论像素之间的邻接、连通、区域与边界，最后说明图像运算究竟作用于像素值、局部邻域、空间坐标还是变换域。后续的灰度变换、空间滤波和频域滤波都建立在这些约定之上。#cite(<dip-book>)
...
$
其中 $N$、$M$ 分别是水平方向和垂直方向的采样数，$cal(L)$ 是允许的像素值集合。灰度图的函数值是一个标量，彩色图的函数值通常是由多个通道组成的向量。]

将所有函数值按行排列，就得到 $M times N$ 矩阵。数学上常写 $f(x,y)$，其中 $x$ 指水平方向、$y$ 指垂直方向；NumPy 数组却先写行再写列，因此访问同一像素使用 `image[y, x]`。混淆两套记号会让宽高、平移方向与坐标点全部交换。
...
图像坐标原点通常位于左上角，$x$ 向右增加，$y$ 向下增加。这与解析几何中 $y$ 轴向上的直角坐标系不同。若图像宽为 `width`、高为 `height`，合法坐标满足 $0<=x<"width"$、$0<=y<"height"$；右下角像素是 `(width - 1, height - 1)`。
