...
人眼看到的是连续场景，计算机处理的却是有限数组。把连续空间变成离散坐标、把连续亮度变成有限数值之后，图像才成为可以存储和计算的数据。本章先建立数字图像的矩阵模型，再讨论像素之间的邻接、连通、区域与边界，最后说明图像运算究竟作用于像素值、局部邻域、空间坐标还是变换域。后续的灰度变换、空间滤波和频域滤波都建立在这些约定之上。#cite(<dip-book>)
