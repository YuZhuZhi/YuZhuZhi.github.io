...
傅里叶变换把图像分解为不同空间频率的复指数分量，频域滤波则进一步决定每种分量应当保留、衰减还是增强。它并不是把频谱图当作普通灰度图随意涂改，而是构造一个与频谱同尺寸的复数乘法因子：每个频率位置的系数同时作用于该频率的幅度和相位，最后再通过逆变换回到空间域。#cite(<dip-book>)

本章沿用上一章的二维 DFT 约定。中心化频谱坐标记为 $(u,v)$，频谱中心为 $(P/2,Q/2)$；径向距离记为
$
  D(u,v)=sqrt((u-P/2)^2+(v-Q/2)^2).
$
实际数组索引和数学频率坐标要始终配套：若频谱已经 `fftshift`，滤波器中心也必须位于数组中央；若频谱未中心化，低频则分布在四个角部。
...
这个公式包含三层意义。第一，乘法是频率位置一一对应的元素乘法，不是矩阵乘法。第二，$H(u,v)=1$ 表示该频率原样通过，$H=0$ 表示完全阻断，介于 0 与 1 之间表示衰减，大于 1 表示增强；复数 $H$ 还可以改变相位。第三，逆变换的理论结果在共轭对称条件下应为实数，代码中出现极小虚部通常来自浮点舍入，可以取实部，但较大的虚部往往说明掩膜破坏了共轭对称或中心化步骤不一致。
...
2. 对直接按频率公式设计的低通、高通滤波器，没有一个有限空间核可直接给出最小尺寸。常见做法是把图像补到至少 $2M times 2N$，使图像周期副本彼此分开，减轻左右、上下边界突然相接造成的环绕影响，逆变换后再裁取原区域。
