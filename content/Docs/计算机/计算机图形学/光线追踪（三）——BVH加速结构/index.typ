...
BVH 是“对象划分”：每个图元属于某个叶节点，空间可以重叠。它不同于均匀网格、八叉树或 k-d tree 等“空间划分”结构。BVH 对动态对象、尺度差异大的场景和通用图元都很友好，因此在离线与实时光追中都很常见。还应看到，BVH 节点实现的是与球体、四边形同一个 `Hittable` 接口，不改变命中记录的字段；这正是后续四边形与光源能不加改动地放进 BVH 的原因。
