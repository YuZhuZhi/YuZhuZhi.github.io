...
- 左上角加入「返回列表页」链接，指向演示文稿所属目录的列表页（如 `content/Notes/测试/`
  → `/Notes/`，文字取 `config.typ` 中 `header-links` 的显示名称，全屏时隐藏）。
- impress.js 记录当前页时由 `location.hash = ...` 改为 `location.replace(...)`。
  上游写法每翻一页都会新增一条浏览器历史记录，于是「后退」只能退回上一张幻灯片，
  需要按很多次才能离开演示文稿；改成 replace 后 URL 仍显示当前页（深链接照常可用），
  但后退键一次就能回到进入前的页面。
